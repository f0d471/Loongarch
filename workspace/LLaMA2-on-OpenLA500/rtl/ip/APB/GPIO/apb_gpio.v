...
// |-------|------|-------|--------|-------------|------------------------------------|
// | GPDAT | 0x00 | 16    | RW     | 0x0000      | GPIO Data Register                 |
// | GPDIR | 0x04 | 16    | RW     | 0x0000      | GPIO Direction Register 0-in 1-out |
...
GPCORE u_GPCORE(
  ...
  .iWDAT (PWDATA[15:0]),    //input [15:0] 写数据 		  
  .iRADR (gpio_addr),       //input [7:0]  读地址		  
  .oRDAT (gpio_rdata),      //output[15:0] 读数据		         
  .iGPIN (iGPIN),           //input [15:0] GPIO输入端口		  
  .oGPOUT(oGPOUT),          //output[15:0] GPIO输出端口		  
  .oINT  (oINT),            //output	   中断输出     
  .oERR  (oERR)             //output	   错误输出      
);

assign PRDATA = {16'h0, gpio_rdata};
